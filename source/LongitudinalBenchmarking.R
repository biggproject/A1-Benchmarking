longitudinal_benchmarking<- function(buildingSubject, buildingsRdf, timeseriesObject, settings, libraryPath="", updateHadoopStatus=F){
  ...
  source(paste0(if(libraryPath!=""){paste0(library,"/")}else{""},"HourlyDynamicModel.R"))
  ...
  for (j in 1:length(settings$MeasuredPropertiesDeviceAggregators)){
    ...
    tryCatch({
      ...
      if(updateHadoopStatus==T){
        write(sprintf("reporter:status: RUNNING %s %s", buildingSubject, measuredPropertyOutput))
      }
      ...
      if(updateHadoopStatus==T){
        write(sprintf("reporter:status: READING DATA %s %s", buildingSubject, measuredPropertyOutput))
      }
      ...
      if(updateHadoopStatus==T){
        write(sprintf("reporter:status: GENERATING REAL INDICATORS %s %s", buildingSubject, measuredPropertyOutput))
      }
      ...
      for(modelName in launcherModes[(launcherModes %in% "HourlyDynamicModel")]){
        ...
        if(updateHadoopStatus==T){
            write(sprintf("reporter:status: GENERATING ESTIMATED INDICATORS %s %s", buildingSubject, measuredPropertyOutput))
        }
        ...
        call <- paste0(
          ...
          paste0("(df=df, identifier=paste(buildingId,deviceAggregator,sep='~'),
                                settings=settings, tz=tz, plots = ",settings$MLFlow$IncludeArtifacts,")")
        )
      ...
      if(updateHadoopStatus==T){
        write(sprintf("reporter:status: SUCCESS %s %s", buildingSubject, measuredPropertyOutput))
      }
    }, error=function(e){
      write(sprintf("###### Error ###### %s",e),stderr())
      if(updateHadoopStatus==T){
        write(sprintf("reporter:status: ERROR %s %s", buildingSubject, measuredPropertyOutput))
      }
